...
本次 driver 内部独立重复评分／容量与投影核验覆盖 157 次执行、1333 个 refinement stages，均成功。32 个源文件／协议／测试哈希、23 个输入哈希在运行前后保持一致；每个 case 子目录保存完整 hierarchy、各方法 JSON 和 NPZ 状态。

[独立审计](audit.json) 首次运行即通过，`errors=[]`；[首轮原件](audit-attempt01.json) 另行保留。审计不调用生产评分器，覆盖：

- 10 份 hierarchy、80 次 contraction，1333 个 stage 端点及层间投影；独立评分 3020 个保存的 assignment 记录。
- 2466 个 IER rounds、296,310 个已记录 selector 的原生评分及容量（60,848 个共享候选，235,462 个后端候选；去重后实际重算 291,379 个 round-selector 状态）。
- 40 个计时内 fallback、147 次从共同粗层标签的 V-cycle、10 次 Flow 自身 continuation／精确状态固定点。
- 全部 120 个 deadline 检查点，确认 30 个超过最大预算的完整返回均被排除。
- 32 个源文件、23 个输入、100 个产物文件哈希，以及 3177 个 assignment 数组内容哈希。

这补齐了上一轮未保存中间层状态、无法独立重放的缺口。但审计没有重放 Flow 内部逐移动轨迹、没有重新优化 FEM 梯度轨迹，也未穷举 IBM 的全部 selector 空间。截止语义依据已存单调时钟事件和冻结源代码核验，保存记录不能单独证明系统调度或机器完全没有其他负载。
